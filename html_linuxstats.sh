...
html="/tmp/health_reports/Server-Health-Report-`hostname`-`date +%y%m%d`-`date +%H%M`.html"
...
#cat $html | mail -s "`hostname` - Daily System Health Report" -a "MIME-Version: 1.0" -a "Content-Type: text/html" -a "From: sricharan@robin.io" $email_add
